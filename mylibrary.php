...
function getECGwaveform($vid)
{
    $db = new Db();
    $query="select '',number*effectiveDuration/numberOfFrames as time,ecg from sonogram inner join 	us_videoclip on sonogram.videoclipid=us_videoclip.videoclipid where sonogram.videoclipid=".$vid;
    $rows = $db -> select($query);
    $rn = count($rows,COUNT_NORMAL);
    return $rows;
    }
