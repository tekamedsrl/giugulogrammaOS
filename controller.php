...
<!-- Questo software e' composto da una web application in PHP su MySQL. Lo scopo 
     e' quello di amministtrare i dati raccolti durante le indgini diagnostiche
     realtive al polso giugulare.
     ...
     Copyright (c) <2017>  <Francesco Sisini> (francescomichelesisini AT gmail com),
     Copyright (c) <2018> <Tekamed SRL>
-->
*/
<?php 
...
/*Inserisce il video*/
	...
	$query=$query." `fileName`) VALUES ($videon,'$pid',now(),now(),'$lor',$jpos,1,1,1,1,1,1,1)";
        //print($query);
        print ("Created videoclip ".$videon."\n");
    }
    $db -> query($query);
    $vid=$db->lastid();
    ...
    for($i=0;$i<$eln-1;$i++)
    {
        if($ecg==1)$ev=$e[$i];else $ev=-1;
        $query="INSERT INTO `sonogram`( `videoclipID`, `number`, `CSA`,`ecg`,`processID`,`perimeter`) VALUES";
        $query=$query."('$vid',$x[$i],$y[$i],$ev,1,$y[$i])";
        $db->query($query);
        //echo $query;
    }
    print ("Inserted ".$i." CSA data\n");
    echo "SUCCESS";
